...
// global variables
string first_file = "/home/leomorningrain/code/orb_cv_ch7/1.png";
string second_file = "/home/leomorningrain/code/orb_cv_ch7/2.png";
...
// -------------------------------------------------------------------------------------------------- //
// ORB pattern:数据集表示以关键点为中心,[-13,12]范围内，随机选点对p，q。这两个点的坐标从数据集上选择，然后乘上之前求的角度再
// 加上关键点，以此找到关键点附近的两个随机像素，然后比较像素值，最终形成描述子
// 比如第一行8,-3,9,5表示两个点为p（8,-3），q(9,5),然后对比两个点坐标对应的像素值大小
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors)
{
    ...
    for (auto &kp : keypoints)
    {
        ...
        {
            bad_points++;
            descriptors.push_back({}); // 如果关键点离边界太近，则描述子为空的vector
            continue;
        }

        // 从左到右，遍历关键点附近半径为8的所有像素点，一共256个像素点
        float m01 = 0, m10 = 0;
        ...
        // compute teh angle of this point
        DescType desc(8, 0);
        for (int i = 0; i < 8; i++)
        {
            uint32_t d = 0;
